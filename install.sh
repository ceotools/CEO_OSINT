...
function Preference {
	...
	read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Language
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT LANGUAGE\n(1)ENGLISH\n(2)ITALIANO\n(3)FRANÇAIS\n(4)BROWSER-LANGUAGE\n\n"
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Language
	...
	read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Theme
	while [ "$Language" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR GUI-DEFAULT THEME\n(1)LIGHT\n(2)DARK\n(3)HIGH-CONTRAST\n\n"
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Language
...
function Mail_Options {
	...
	read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Opt
	while [ "$Opt" = "" ];
		do
        printf "${BLUE}\n[?]${WHITE}WOULD YOU LIKE TO ENABLE EMAIL-OPTION(1)YES(2)NO\n\n"
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Opt
	...
	if [ $Opt == 1 ];
		...
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" recipient
		while [ "$recipient" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR RECIPIENT EMAIL\n\n"
			read -p"$GREEN[#CEO_OSINT#]$WHITE-->" recipient
		...
		read -sp"$GREEN[#CEO_OSINT#]$WHITE-->" password
		while [ "$password" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR EMAIL PASSWORD\n\n"
			read -sp"$GREEN[#CEO_OSINT#]$WHITE-->" password
		...
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" destination
		while [ "$destination" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR DESTINATION EMAIL\n\n"
			read -p"$GREEN[#CEO_OSINT#]$WHITE-->" destination
		...
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" server
		while [ "$server" = "" ];
			do
			printf "${WHITE}\nINSERT YOUR SMTP SERVER EX smtp.test.com\n\n"
			read -p"$GREEN[#CEO_OSINT#]$WHITE-->" server
		...
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" port
		while [ "$port" = "" ];
			do
...
function Options {
	...
	read -sp"$GREEN[#CEO_OSINT#]$WHITE-->" up_pass
	while [ "$up_pass" = "" ];
		do
        printf "${WHITE}\nINSERT YOUR UPDATE-PASSWORD \n\n"
        read -sp"$GREEN[#CEO_OSINT#]$WHITE-->" up_pass
	...
	read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Token
	while [ "$Token" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ACCESS YOUR DATABASE ON OTHER DEVICES(ON THE SAME NETWORK)?(1)YES(2)NO\n\n"
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Token
	...
	read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Access
	while [ "$Access" == "" ];
		do
		printf "${WHITE}\n\nWOULD YOU LIKE TO ADD SOME CREDENTIALS FOR ACCESS THE DATABASE?(1)YES(2)NO\n\n"
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" Access
	...
	read -p"$GREEN[#CEO_OSINT#]$WHITE-->" DateFormat
	while [ "$DateFormat" == "" ];
		do
		printf "${WHITE}\nSELECT YOUR DATE-FORMAT\n(1)EUROPE(DD/MM/YYYY)\n(2)AMERICA'USA'(MM/DD/YYYY)\n(3)ASIA(YYYY/MM/DD)\n\n"
		read -p"$GREEN[#CEO_OSINT#]$WHITE-->" DateFormat
	...
	cd Configuration
	echo ";THIS FILE HAS BEEN GENERATE BY CEO_OSINT INSTALLER">Configuration.ini
...
echo '{
	...
	echo ";THIS FILE HAS BEEN GENERATE BY CEO_OSINT INSTALLER">Configuration/Configuration.ini
	...
	echo "password = CEO">>Configuration/Configuration.ini
	...
	printf "\n${WHITE}UPDATE-PASSWORD:${GREEN}CEO\n"
...
function installer {
	...
	if [ $confvar == 1 ];
		...
		read -p "$GREEN[#CEO_OSINT#]$WHITE-->" selected
		while [ "$selected" == "" ];
			do
			printf "${BLUE}\n\nWOULD YOU LIKE TO SET(1)MANUAL-INSTALLATION(2)AUTO-INSTALLATION\n\n"
			read -p "$GREEN[#CEO_OSINT#]$WHITE-->" selected
		...
		printf "${LIGHTGREEN}\n\nTHANK YOU FOR HAVE INSTALLED CEO_OSINT\n\n"
		exit 0
	fi
